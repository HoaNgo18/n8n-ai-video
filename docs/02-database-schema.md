...
| 4 | `Status` | System | Enum | `Pending` -> `In Progress` -> `Draft` -> `Approved` / `Rejected` -> `Published` / `Failed`. |
...
| 9 | `Audio_Timing` | Phase 3A | JSON string | Timing manifest per segment/screenshot, example: `[{"image_index":0,"start":0.3,"end":4.7}]`. |
...
| 18 | `Note` | Any | Text | Debug note, quality note, or error context. Phase 1 now stores engagement, fit score, AI angle, and reject/keep reason here. |
